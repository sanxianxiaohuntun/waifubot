...
#### 0.0.1.1更新 时间 2025/07/29 01:44
新增人设卡 生成后导入功能
修复 webui 的所有已知bug
配置中加入 模型生成创意度调整功能

#### 0.0.1.2更新 时间 2025/07/29 03:16
修复了 一键导入人设卡需要输入群聊\私聊前续的问题
...
某些api并不支持结构化返回，这时候 ##清空记忆 命令进行清空记忆更换模型即可。

如果需要禁用某些工具调用比如：绘图、语音、音乐，直接在人设卡最后告诉LLM不要进行调用即可，比如：**禁止使用**：语音、音乐、绘图功能。

#### 0.0.1.3 更新 时间 2025/08/02 09:00
1.webui 生成角色卡部分 持久化记忆 用户不删除不会重置，并且可以重新生成和复制内容了。
2.webui把图片和语音生成功能设置了开关，关闭后语音会通过文本发送，绘图将不会生成。
3.完成了LLM1和LLM2模式的完整架构。LLM1模式 token会增加1000-2000但是不会在出现需求 模型能够结构化请求，LLM2模式 token较少，但是需要API站点和模型支持结构化请求
4.可以自定义破甲文本了，新增LLM提示词修改。TOP部分为顶部注入，END部分为聊天记录和用户当前输入下方注入用于伪造AI回复。
5.修复了若干bug。
6.更新了酒馆模式（第三人称 需要角色卡是第三人称），助手模式（完全多模态辅助用户完成内容），角色扮演模式（传统waifu模式）

#### 0.0.1.4 更新 时间 2025/08/03 03:50
1.解决返回不支持 XML和JSON报错问题，增加重试机制。
2.解决百度的deepseek v3不允许非user内容结尾问题。
3.减少长对话下的token消耗，稳定输出。
4.增加异常处理机制，更好的异常处理。

#### 0.0.1.5 更新 时间 2025/08/04 22:14
1.修复模型返回表情包错误以及格式化错误问题
2.系统不再对大小写敏感
3.修复若干bug
4.对便宜模型兼容效果更好了
5.解决了杀毒软件报毒问题
6.兼容了deepseek全模型的格式问题

#### 0.0.1.6 更新 时间 2025/08/05 03:12
1.解决了模型在群聊中一次回复多个人问题，现在不同回复某个人的话会@不同的人了。
2.解决了群聊中 先触发bot在说问题的事情，现在会延迟等待10-20秒，没后续消息才触发LLM 最短10秒最多20秒。
3.解决了 bot会忘记自己状态 导致绘图场景不一致和跳跃式位置问题。

#### 0.0.1.7 更新 时间 2025/08/06 05:03
1.修复了群聊中等待时间的逻辑问题。
2.webui 增加了群聊等待时间配置 在群聊/用户管理和默认模板配置中。
3.优化了提示词结构。
4.修复了若干bug。

#### 0.0.1.8 更新 时间 2025/08/07 11:33
1.完成了酒馆模式和助手模式的设计，允许进行非第一人称回复了。

#### 0.0.1.9 更新 时间 2025/08/08 16:28
1.新增了记事本功能，bot会通过记事本记录内容了，你让它不要忘记，记住的东西它会记录到记事本永久记忆，通过向量化文本然后向量+弧相似度+关键词召回最相关的50条，bot在也不忘记重要事件了。

#### 0.0.2.0 更新 时间 2025/08/09 14:26
1.新增了记事本功能，bot会通过记事本记录内容了，你让它不要忘记，记住的东西它会记录到记事本永久记忆，通过向量化文本然后向量+弧相似度+关键词召回最相关的50条，bot在也不忘记重要事件了。

2.记事本可以再 webui里删除对应内容了。
