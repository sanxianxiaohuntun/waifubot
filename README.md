...
1.  **🔐 身份验证系统 (全新)**：
    *   WebUI 登录系统，首次启动需设置管理员账号密码 [i].
    *   支持密码修改和会话持久化 [i].

2.  **🔑 API 密钥管理系统 (升级)**：
    *   一键更新所有 API Key 字段 [i].
    *   专为启航 API 优化，支持 API 有效性验证和余额查询 [i].

3.  **🎨 模型选择系统 (全新)**：
    *   智能筛选 LLM、搜索、TTS 和图像模型 [i].
    *   高级配置界面，下拉选择模型，实时从 API 获取最新模型列表 [i].

4.  **🎵 语音功能系统 (独立模块)**：
    *   TTS 测试，自定义文本转语音预览和实时试听 [i].
    *   支持 MP3 格式下载，界面预留语音克隆功能（未实装）[i].

5.  **🖼️ 图像生成系统 (升级)**：
    *   绘图配置独立化，支持实时预览、图片保存和重新生成 [i].
    *   支持 NSFW 模型配置，提供二次元/三次元风格预览 [i].
...
7.  **⚡ 性能优化 (技术升级)**：
    *   图片自动压缩和格式转换，优化音频播放体验 [i].
    *   优化构建流程 [i].

8.  **🐛 问题修复 (稳定性提升)**：
    *   修复音频播放、滚动行为和样式一致性问题 [i].
    *   增强安全，严格验证用户名密码格式 [i].

9.  **🎯 用户体验提升**：
    *   一键配置 API 密钥，简化配置流程 [i].
    *   智能提示和实时验证，减少手动输入错误 [i].
    *   增强 WebUI 登录保护和密码管理 [i].
